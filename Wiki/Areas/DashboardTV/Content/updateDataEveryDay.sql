...
SELECT
MSP_EpmProject_UserView.[№ заказа]
,isnull(convert(int,sum([PortalKATEK].[dbo].[PZ_TEO].SSM / [ReportKATEK].[dbo].[ProjectWorkPO].[AssignmentWork] * ProjectWebApp.dbo.MSP_EpmProject_UserView.[Кол-во ПО] * ProjectWebApp.dbo.MSP_EpmAssignmentByDay_UserView.AssignmentWork)), 0)
...
FROM
ProjectWebApp.dbo.MSP_EpmProject_UserView
 join [PortalKATEK].[dbo].[PZ_PlanZakaz] on convert(varchar,[PortalKATEK].[dbo].[PZ_PlanZakaz].PlanZakaz) = MSP_EpmProject_UserView.[№ заказа]
INNER JOIN
ProjectWebApp.dbo.MSP_EpmTask_UserView
ON
ProjectWebApp.dbo.MSP_EpmProject_UserView.ProjectUID = ProjectWebApp.dbo.MSP_EpmTask_UserView.ProjectUID
