...
{
    public class EmailArmis : EmailClient
    {
        ...
        public EmailArmis(CMOSOrder order, string login)
        {
            try
            {
                ...
                GetBody();
                SendEmail();
                logger.Debug("EmailArmis: " + order.id);
            }
            catch (Exception ex)
            {
                logger.Error("EmailArmis: " + order.id + " | " + ex);
            }
        }

        bool GetMailPM()
        {
            mailToList.Add("armis@katek.by");
        ...
        {
            return Directory.GetFiles(@"\\192.168.1.30\m$\_ЗАКАЗЫ\CMOS\Armis\" + order.id.ToString() + @"\").ToList();
        }
    }
}