...
namespace Wiki.Areas.CMO.Models
{
    public class EmailCMO : EmailClient
    {
        ...
        {
            string typeName = "";
            foreach (var data in order.CMO2_Position.OrderBy(d => d.CMO_TypeProduct.name).ToList())
            {
                typeName += data.CMO_TypeProduct.name + ", ";
            }
            typeName = typeName.Substring(0, typeName.Length - 2);
            string pzs = "";
            foreach (var data in order.CMO2_Position.OrderBy(d => d.PZ_PlanZakaz.PlanZakaz).ToList())
            {
                pzs += data.PZ_PlanZakaz.PlanZakaz.ToString() + " - " + typeName + "; ";
            }
